...
def appointment(request, pk):
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            days = request.POST.get("days").split(",")
            date = days[0]
            day = days[1]
            ...
            Nslots = Appointment.objects.filter(Status="P", Service=servicedetaildaytime, day=date).count(
            ) + Appointment.objects.filter(Status="A", Service=servicedetaildaytime, day=date).count()
